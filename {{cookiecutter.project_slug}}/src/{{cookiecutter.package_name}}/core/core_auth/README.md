...
```python
from {{ cookiecutter.package_name }}.core.core_auth.acl import Perm, ResourceACL, check_acl
...
```python
from {{ cookiecutter.package_name }}.core.core_auth.acl import Perm, ResourceACL, require_acl_perm
...
```python
from {{ cookiecutter.package_name }}.core.core_auth.acl import check_acl, Perm, ResourceACL
from {{ cookiecutter.package_name }}.core.core_auth.deps import get_current_user
...
AUTH_JWKS_URL=https://login.gwq-serviceplus.de/realms/GWQ/.well-known/jwks.json
AUTH_ISSUER=https://login.gwq-serviceplus.de/realms/GWQ
...
from {{ cookiecutter.package_name }}.core.core_auth import UserClaims, require_write, require_admin

router = APIRouter(prefix="/muster")

@router.post("/")
async def create_muster(
    payload: CreateMusterInput,
    user: UserClaims = Depends(require_write),
):
    ...

@router.delete("/{id}")
async def delete_muster(
    id: int,
...
```python
from {{ cookiecutter.package_name }}.core.core_auth.deps import require_roles
...
```python
from {{ cookiecutter.package_name }}.core.core_auth.deps import get_value_from_jwt, Credentials
