...
from {{ cookiecutter.package_name }}.core.core_auth.acl import Perm, ResourceACL, check_acl, require_acl_perm
from {{ cookiecutter.package_name }}.core.core_auth.deps import get_current_user, require_roles
from {{ cookiecutter.package_name }}.core.core_auth.models import UserClaims
from {{ cookiecutter.package_name }}.core.core_auth.roles import extract_groups, extract_roles, get_effective_roles, parse_hierarchy
from {{ cookiecutter.package_name }}.core.core_auth.service import JWTAuthService
from {{ cookiecutter.package_name }}.core.core_auth.settings import AuthSettings, RoleSettings
from {{ cookiecutter.package_name }}.shared.types import JSONValue
...
class TestExtractRoles:
    def test_top_level_roles_array(self) -> None:
        with patch("{{ cookiecutter.package_name }}.core.core_auth.roles.get_role_settings", return_value=RoleSettings(PREFIX="")):
    ...
    def test_prefix_stripped_from_roles(self) -> None:
        ...
        with patch("{{ cookiecutter.package_name }}.core.core_auth.roles.get_role_settings", return_value=RoleSettings(PREFIX="GRPS_")):
            result = extract_roles(payload)
...
class TestJWTAuthServiceValidation:
    def test_valid_rs256_token_decoded(self, rsa_key: Any) -> None:
        ...
        with patch("{{ cookiecutter.package_name }}.core.core_auth.service.get_jwks_client", return_value=mock_client):
...
class TestGetCurrentUser:
    ...
    def test_valid_token_returns_user_claims(self, rsa_key: Any) -> None:
        ...
        async def _run() -> UserClaims:
            with patch("{{ cookiecutter.package_name }}.core.core_auth.deps.validate_jwt", return_value=valid_payload):
    ...
    def test_invalid_jwt_raises_401(self) -> None:
        ...
        async def _run() -> None:
            with patch("{{ cookiecutter.package_name }}.core.core_auth.deps.validate_jwt", side_effect=HTTPException(401, "invalid_token")):
...
class TestRequireRoles:
    ...
    def test_user_with_required_role_passes(self) -> None:
        ...
        async def _run() -> UserClaims:
            with patch("{{ cookiecutter.package_name }}.core.core_auth.deps.get_role_settings", return_value=role_settings):
...
class TestRequireRolesWithHierarchy:
    ...
    def test_editor_inherits_read_permission(self) -> None:
        ...
        async def _run() -> UserClaims:
            with patch("{{ cookiecutter.package_name }}.core.core_auth.deps.get_role_settings", return_value=cfg):
...
class TestExtractGroups:
    ...
    def test_prefix_stripped_from_groups(self) -> None:
        ...
        with patch(
            "{{ cookiecutter.package_name }}.core.core_auth.roles.get_role_settings", return_value=RoleSettings(PREFIX="GRPS_Portal_DocuFlow_")
        ):
            result = extract_groups(payload)
