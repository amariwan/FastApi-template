...
def _template_int(value: str) -> int:
    return int(value.strip())


def _template_list(value: str) -> list[str]:
    return _split_env_values(value)


def _template_optional_str(value: str) -> str | None:
    cleaned = value.strip()
...
class AuthSettings(BaseSettings):
    ...
    MODE: StrictStr = Field(default="{{ cookiecutter.default_auth_mode }}", description="jwks | hs")
    VALIDATE_SIGNATURE: bool = Field(
        default_factory=lambda: _template_bool("{{ cookiecutter.auth_validate_signature }}"),
        description="Enable full JWT signature validation",
    )
    VERIFY_SIGNATURE: bool = Field(default_factory=lambda: _template_bool("{{ cookiecutter.auth_verify_signature }}"))
    VERIFY_EXP: bool = Field(default_factory=lambda: _template_bool("{{ cookiecutter.auth_verify_exp }}"))
    VERIFY_ISS: bool = Field(default_factory=lambda: _template_bool("{{ cookiecutter.auth_verify_iss }}"))
    VERIFY_AUD: bool = Field(default_factory=lambda: _template_bool("{{ cookiecutter.auth_verify_aud }}"))
    DISABLE_SSL_VERIFY: bool = Field(default_factory=lambda: _template_bool("{{ cookiecutter.auth_disable_ssl_verify }}"))
    CLOCK_SKEW_SECS: StrictInt = Field(
        default_factory=lambda: _template_int("{{ cookiecutter.auth_clock_skew_secs }}"),
        ge=0,
        le=900,
    )
    ...
    def _parse_clock_skew_secs(cls, value: object) -> object:
        ...
        if value is None or value == "":
            return _template_int("{{ cookiecutter.auth_clock_skew_secs }}")
    ...
    ALGORITHMS: list[StrictStr] | StrictStr = Field(
        default_factory=lambda: _normalize_algorithms(_template_list("{{ cookiecutter.default_auth_algorithms }}"))
    )
    ...
    def _parse_algorithms(cls, value: object) -> list[str]:
        if value is None or value == "":
            return _normalize_algorithms(_template_list("{{ cookiecutter.default_auth_algorithms }}"))
        if isinstance(value, str):
...
class RoleSettings(BaseSettings):
    ...
    ACTIVE: bool = True
    ACTIVE: bool = Field(default_factory=lambda: _template_bool("{{ cookiecutter.role_active }}"))
    PREFIX: StrictStr = "{{ cookiecutter.role_prefix }}"
    READ_ROLES: list[StrictStr] = Field(default_factory=lambda: [entry.lower() for entry in _template_list("{{ cookiecutter.role_read_roles }}")])
    WRITE_ROLES: list[StrictStr] = Field(default_factory=lambda: [entry.lower() for entry in _template_list("{{ cookiecutter.role_write_roles }}")])
    DELETE_ROLES: list[StrictStr] = Field(default_factory=lambda: [entry.lower() for entry in _template_list("{{ cookiecutter.role_delete_roles }}")])
    ADMIN_ROLES: list[StrictStr] = Field(default_factory=lambda: [entry.lower() for entry in _template_list("{{ cookiecutter.role_admin_roles }}")])
    HIERARCHY: StrictStr | None = Field(
        default_factory=lambda: _template_optional_str("{{ cookiecutter.role_hierarchy }}"),
        description=(
