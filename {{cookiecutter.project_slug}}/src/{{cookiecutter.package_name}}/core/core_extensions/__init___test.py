...
from {{ cookiecutter.package_name }}.core.core_extensions import loader
from {{ cookiecutter.package_name }}.core.core_extensions.loader import (
    RuntimeService,
    ServiceRegistration,
...
from {{ cookiecutter.package_name }}.core.core_messages import MessageKeys, msg
...
def test_discover_service_module_names_returns_sorted_service_integrations(tmp_path: Path) -> None:
    ...
    assert modules == [
        "{{ cookiecutter.package_name }}.services.docgen.integration",
        "{{ cookiecutter.package_name }}.services.docmanager.integration",
    ]


def test_get_service_registrations_returns_loaded_services(monkeypatch: pytest.MonkeyPatch) -> None:
    router = APIRouter()
    monkeypatch.setattr(loader, "discover_service_module_names", lambda: ["{{ cookiecutter.package_name }}.services.docmanager.integration"])
    monkeypatch.setattr(
        loader,
...
def test_get_service_registrations_raises_when_no_service_loaded(
    ...
    assert str(exc_info.value) == msg.get(
        ...
        modules="{{ cookiecutter.package_name }}.services.docmanager.integration",
    )
