...
from {{ cookiecutter.package_name }}.core.core_storage.base import FileMetadata
from {{ cookiecutter.package_name }}.core.core_storage.exceptions import StorageConfigError, StorageError, StorageFileNotFoundError
from {{ cookiecutter.package_name }}.core.core_storage.s3 import S3StorageClient
from {{ cookiecutter.package_name }}.core.core_storage.settings import StorageBackend, StorageSettings
...
def test_upload_returns_metadata() -> None:
    ...
    assert call_kwargs["Key"] == "muster/v1/doc.docx"
    assert call_kwargs["Body"] == b"hello"

    assert isinstance(meta, FileMetadata)
    assert meta.path == "muster/v1/doc.docx"
...
def test_download_returns_bytes() -> None:
    ...
    result = client.download_file("muster/v1/doc.docx")

    s3.get_object.assert_called_once_with(Bucket="test-bucket", Key="muster/v1/doc.docx")
