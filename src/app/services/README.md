...
1. **Placement:** The service directory must be located under `app/services/<service_name>/` and must contain an `integration.py` file.
...
6. **No Side Effects on Import:** Importing the `app/services/<service_name>/integration.py` module must not establish connections or execute expensive initializations. All connections/clients must be initialized within the startup hooks.
...
- **Discovery:** The Core Loader searches `app/services/` for subdirectories containing an `integration.py` and imports `app.services.<service_name>.integration`.
...
- **Package:** `app.services.<service_name>`
- **Integration file:** `app/services/<service_name>/integration.py`
- **API router:** `app/services/<service_name>/api/router.py` with `router = APIRouter(...)`
...
1. Create a new directory: `app/services/<service_name>/`
...
_(Translated example payload for clarity)_
